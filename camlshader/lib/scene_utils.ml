...
let cross a b =
  let x = (Mat.get a 1 0 *. Mat.get b 2 0) -. (Mat.get a 2 0 *. Mat.get b 1 0) in
  let y = (Mat.get a 2 0 *. Mat.get b 0 0) -. (Mat.get a 0 0 *. Mat.get b 2 0) in
  let z = (Mat.get a 0 0 *. Mat.get b 1 0) -. (Mat.get a 1 0 *. Mat.get b 0 0) in
...
let translate_camera (dir : translation_dir) (speed : float) (state : scene_state) =
