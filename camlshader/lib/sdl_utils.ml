...
let build_window name w h =
    match Sdl.create_window ~w:w ~h:h name Sdl.Window.(shown + resizable + metal + allow_highdpi)
    with
    | Error (`Msg e) ->
...
let build_renderer win =
  match Sdl.create_renderer win ~index:0 ~flags:Sdl.Renderer.(accelerated + presentvsync) with
  | Error (`Msg e) -> Sdl.log "failed to create renderer : %s" e; None
  | Ok ren -> Some ren
;;

let build_texture ren w h =
  match Sdl.create_texture ren Sdl.Pixel.format_abgr8888 Sdl.Texture.access_streaming ~w:w ~h:h with
  | Error (`Msg e) -> Sdl.log "failed to create texture : %s" e; None
...
let build_default_sdl_state name w h =
  let keys = build_keys 16 in (* default to 16 keys ... *)
  let mouse = build_mouse () in
  if not (start_sdl ()) then None
  else match build_window name w h with
  | None -> Sdl.quit (); None
  | Some win -> (
    match build_renderer win with
    | None -> Sdl.destroy_window win; Sdl.quit (); None
    | Some ren -> (
      match build_texture ren w h with
      | None -> Sdl.destroy_renderer ren; Sdl.destroy_window win; Sdl.quit(); None
...
let render_texture state pxls pitch =
  ...
  | Error(`Msg e) -> Sdl.log "failed to update texture: %s" e; ()
  | Ok () ->
...
let handle_keydown state keycode = Hashtbl.replace state.keys keycode true; state
let handle_keyup state keycode = Hashtbl.replace state.keys keycode false; state