...
type keyboard_action = {
  key : Tsdl.Sdl.keycode;
  act : app_state -> app_state;
}
...
let default_actions =
  ...
  { key = Sdl.K.w; act = fun state -> { state with scenes = translate_camera Forward 0.1 state.scenes } };
  { key = Sdl.K.a; act = fun state -> { state with scenes = translate_camera Left 0.1 state.scenes} };
  { key = Sdl.K.s; act = fun state -> { state with scenes = translate_camera Backward 0.1 state.scenes } };
  { key = Sdl.K.d; act = fun state -> { state with scenes =  translate_camera Right 0.1 state.scenes } };
  { key = Sdl.K.e; act = fun state -> { state with sdls = (Sdl_utils.toggle_grab_mouse state.sdls) } }
]
;;
...
let apply_mouse_actions state =
  ...
  if dx != 0 || dy != 0 then begin 
    ...
    state.sdls.mouse_delta := (0, 0); (* CAUTION: this is sort of not safe ... .. . *)
